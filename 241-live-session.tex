...
\begin{figure}
\centering
\includegraphics{./images/point_reyes.jpg}
...
\begin{verbatim}
##    id         y0        y1
## 1:  1 0.50034209 0.7503421
## 2:  2 0.25456662 0.5045666
## 3:  3 0.11247068 0.3624707
## 4:  4 0.08702677 0.3370268
## 5:  5 0.32121013 0.5712101
\end{verbatim}
...
## 1:  1 0.50034209 0.7503421                       0
## 2:  2 0.25456662 0.5045666                       0
## 3:  3 0.11247068 0.3624707                       0
## 4:  4 0.08702677 0.3370268                       0
## 5:  5 0.32121013 0.5712101                       1
\end{verbatim}
...
## 1:  1 0.50034209 0.7503421                       0                       0
## 2:  2 0.25456662 0.5045666                       0                       1
## 3:  3 0.11247068 0.3624707                       0                       1
## 4:  4 0.08702677 0.3370268                       0                       1
## 5:  5 0.32121013 0.5712101                       1                       1
\end{verbatim}
...
\includegraphics{241-live-session_files/figure-latex/unnamed-chunk-8-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(sandwich) }\CommentTok{\# estimates HCE easily}
\FunctionTok{library}\NormalTok{(lmtest)   }\CommentTok{\# sets up t{-}test easily }
\NormalTok{d }\OtherTok{\textless{}{-}} \FunctionTok{data}\NormalTok{(petersen)}
...
\FunctionTok{coeftest}\NormalTok{(m1, }\AttributeTok{vcov =} \FunctionTok{vcov}\NormalTok{(m1))}

\DocumentationTok{\#\# to estimate robust SEs is a two line solution}
\NormalTok{m1}\SpecialCharTok{$}\NormalTok{r.vcov }\OtherTok{\textless{}{-}} \FunctionTok{vcovHC}\NormalTok{(m1)}
\FunctionTok{coeftest}\NormalTok{(m1, }\AttributeTok{vcov =}\NormalTok{ m1}\SpecialCharTok{$}\NormalTok{r.vcov)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m2}\SpecialCharTok{$}\NormalTok{r.vcov }\OtherTok{\textless{}{-}} \FunctionTok{vcovHC}\NormalTok{(m2, }\AttributeTok{type =} \StringTok{"HC3"}\NormalTok{)}
\FunctionTok{coeftest}\NormalTok{(m2, }\FunctionTok{vcovHC}\NormalTok{(m2, }\AttributeTok{type =} \StringTok{"const"}\NormalTok{))}
\FunctionTok{coeftest}\NormalTok{(m2, m2}\SpecialCharTok{$}\NormalTok{r.vcov)}
\end{Highlighting}
\end{Shaded}
...
Another, relatively recently developed package now exists that \emph{shoud} make considerably more simple the task of estimating clustered standard errors. The package has been developed since the recording of the async material, so we'll do the work to present it here.
...
\NormalTok{makeData }\OtherTok{\textless{}{-}} \ConstantTok{TRUE}

\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}

\ControlFlowTok{if}\NormalTok{(makeData) \{}
\NormalTok{    sim\_size }\OtherTok{\textless{}{-}} \DecValTok{1000}
\NormalTok{    d }\OtherTok{\textless{}{-}} \FunctionTok{data.table}\NormalTok{(}\AttributeTok{californian =} \FunctionTok{sample}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{), sim\_size, }\AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{),}
                    \AttributeTok{affluence =} \FunctionTok{sample}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{7}\NormalTok{, sim\_size, }\AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{),}
                    \AttributeTok{preY =} \FunctionTok{rnorm}\NormalTok{(sim\_size, }\AttributeTok{mean =} \DecValTok{100}\NormalTok{, }\AttributeTok{sd =} \DecValTok{7}\NormalTok{),}
                    \AttributeTok{treat =} \FunctionTok{sample}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{), sim\_size, }\AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{) )}
\NormalTok{    d[ , literacy }\SpecialCharTok{:}\ErrorTok{=} \FunctionTok{rnorm}\NormalTok{(sim\_size, }\AttributeTok{mean =}\NormalTok{ d}\SpecialCharTok{$}\NormalTok{affluence, }\AttributeTok{sd =} \DecValTok{2}\NormalTok{)]}
\NormalTok{    d[ , tau      }\SpecialCharTok{:}\ErrorTok{=} \FunctionTok{rnorm}\NormalTok{(sim\_size, }\AttributeTok{mean =} \DecValTok{5} \SpecialCharTok{+}\NormalTok{ californian)] }\DocumentationTok{\#\# + d$affluence)}
\NormalTok{    d[ , postY    }\SpecialCharTok{:}\ErrorTok{=}\NormalTok{  preY }\SpecialCharTok{+}\NormalTok{ tau}\SpecialCharTok{*}\NormalTok{treat]}
\NormalTok{\}}

\DocumentationTok{\#\# What is the (unobserved) true average treatment effect?}
...
\begin{figure}
\centering
\includegraphics{https://www.youtube.com/watch?v=K_8vta9XDpc}
...
If there are any limitations to what you design, please voice those concerns and talk about why they arise, relative to an \emph{ideal} experiment (that you are proably unable to conduct).
...
\FunctionTok{library}\NormalTok{(lmtest)                 }\CommentTok{\# though we won\textquotesingle{}t actually use them in this}
\FunctionTok{library}\NormalTok{(sandwich)               }\CommentTok{\# and neither this... }
...
\NormalTok{d }\OtherTok{\textless{}{-}} \FunctionTok{data.table}\NormalTok{(}\AttributeTok{id =} \DecValTok{1}\SpecialCharTok{:}\NormalTok{nrows) }

\NormalTok{d[ , y0 }\SpecialCharTok{:}\ErrorTok{=} \FunctionTok{rnorm}\NormalTok{(nrows, }\AttributeTok{mean =} \DecValTok{10}\NormalTok{)]}
\NormalTok{d[ , tau }\SpecialCharTok{:}\ErrorTok{=} \FunctionTok{rnorm}\NormalTok{(nrows, }\AttributeTok{mean =} \DecValTok{5}\NormalTok{)]}
...
\NormalTok{d[assigned }\SpecialCharTok{==} \DecValTok{1}\NormalTok{, treated }\SpecialCharTok{:}\ErrorTok{=} \FunctionTok{sample}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{), }\AttributeTok{size =}\NormalTok{ .N, }\AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{prob =} \FunctionTok{c}\NormalTok{(.}\DecValTok{7}\NormalTok{, .}\DecValTok{3}\NormalTok{))]}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{d2 }\OtherTok{\textless{}{-}}\NormalTok{ d[ , .(assigned, treated, Y)]}
\NormalTok{d2[ , .(}\AttributeTok{mean =} \FunctionTok{mean}\NormalTok{(Y)), by }\OtherTok{=}\NormalTok{ assigned][ , }\FunctionTok{diff}\NormalTok{(mean)]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{5} \SpecialCharTok{*}\NormalTok{ .}\DecValTok{7}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\# rats. we /know/ that the treatment effect is 5, but when we look at the}
\DocumentationTok{\#\# intent to treat effect, we only estimate a difference of 3.5 or so. }
\end{Highlighting}
\end{Shaded}
...
- This requires that you suspect reality for a moment and suppose we don't have the \texttt{treated} measurement.

\hypertarget{estimate-the-intent-to-treat-effect}{%
\subsubsection{Estimate the Intent to Treat Effect}\label{estimate-the-intent-to-treat-effect}}
...
\subsubsection{Do you get the same thing with this subset estimator?}\label{do-you-get-the-same-thing-with-this-subset-estimator}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{d2[assigned }\SpecialCharTok{==} \DecValTok{1}\NormalTok{ , .(}\AttributeTok{group\_mean =} \FunctionTok{mean}\NormalTok{(Y)), by }\OtherTok{=}\NormalTok{ .(treated)] }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{    .[ , }\FunctionTok{diff}\NormalTok{(group\_mean)]}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Doing \emph{exactly} the same thing that the CACE = ITT / ITT\_\{d\}; but,
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{iv.model }\OtherTok{\textless{}{-}} \FunctionTok{ivreg}\NormalTok{(Y }\SpecialCharTok{\textasciitilde{}}\NormalTok{ treated }\SpecialCharTok{|}\NormalTok{ assigned, }\AttributeTok{data =}\NormalTok{ d2)}

\FunctionTok{coeftest}\NormalTok{(iv.model, }\AttributeTok{vcov =} \FunctionTok{vcovHC}\NormalTok{(iv.model, }\AttributeTok{type =} \StringTok{"const"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{241-live-session_files/figure-latex/unnamed-chunk-26-1.pdf}
...
\includegraphics{241-live-session_files/figure-latex/unnamed-chunk-29-1.pdf}
